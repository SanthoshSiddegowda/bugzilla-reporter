...
class BitbucketAPI:
    ...
    def format_prs_for_chat(self, prs: List[Dict]) -> str:
        ...
        # Format each author's PRs
        for author, author_prs in prs_by_author.items():
            message += f"👤 *{author}* ({len(author_prs)} PRs)\n\n"
            
            for pr in author_prs:
                # Format PR title as a clickable link
        ...
        total_prs = sum(len(author_prs) for author_prs in prs_by_author.values())
        message += f"─────────────────────────\n"
        message += f"📊 Total PRs: {total_prs}"
